...
const AllProducts = () => {
  ...
  useEffect(() => {
    ...
    const fetchSubcategories = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:5000/getSubCategory/${category}`);
        const data = response.data;
        if (data.status === 'ok') {
      ...
      <div className="container14 mt-4">
        ...
        <div className="row">
          {subcategories.length > 0 && (
            <div className="col-sm-3 subcategorylist">
              <h5>Subcategories</h5>
              <ul>
          ...
          <div className={subcategories.length > 0 ? 'col-sm-9' : 'col-12'}>
            <div className="row">
